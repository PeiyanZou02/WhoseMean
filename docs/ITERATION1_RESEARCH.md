...
`Generator512` is a six-scale U-Net (32→64→128→256→512→512 channels, InstanceNorm, LeakyReLU down / ReLU up, `tanh` output, skip concatenations, 8×8 bottleneck for a 512 px input). `Discriminator512` is a five-layer PatchGAN over the channel-wise concatenation of source and target (6 input channels). `edge_loss` is an L1 on horizontal and vertical finite differences. This is a faithful, compact pix2pix at 512 px with roughly a few tens of millions of generator parameters — no pretraining, no attention, no text conditioning of any kind.
...
6. **The interaction loop is minutes-to-hours, not seconds.** At 512 px, batch 1, with G+D forward/backward, a single epoch over 1000 works is on the order of minutes on a current consumer GPU; 20 epochs is on the order of an hour. *(Approximate — not benchmarked in this pass; see §11.)* The core gesture of the piece — exclude works, see what changes — is therefore effectively non-interactive. This is the single most serious problem, and it is an HCI problem, not an ML one.
...
7. **Current build's actual epoch wall-clock time.** The minutes-per-epoch and ~1-hour-per-run figures in §1.5 are reasoned estimates from the architecture, resolution and batch size, **not measurements**. Benchmark this in Phase 0 of the plan; the entire latency argument deserves a real number.
